...
const BarChartSkripsi = () => {
  ...
  const getChartData = async () => {
    try {
      const response = await fetch("http://localhost:5000/chart/barskripsi");
  ...
  if (chartData.length != 0) {
    ...
    return (
      <>
        <Bar options={options} data={data} className="max-w-96 h-auto" />
      </>
    );
  }
};

export default BarChartSkripsi;
